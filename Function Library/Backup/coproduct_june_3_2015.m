...
ClearAll[IMPL,MZV,coproduct,maxCoproduct,higherCoproduct,zetaValueCoproduct,transcendentalWeight,sumOverTwo,twoCoproduct,fullCoproduct,coproductTimes,CircleDot,LogsToIMPL,IMPLtoLogs,exteriorD,exteriorDeriv,exteriorDifferentiation,coproductD,allIrreducibleFunctions,irreducibleFunctionCoproduct,shuffleW,toLinearBasis,cycle,exchange,yWeight,yGrading,replace\[CapitalDelta],coproductEntry,uReplacements,yReplacements,yMatchReplacements,Z,expandHPL,compressedNotation,toStrictLyndonBasis];
...
Do[Evaluate[Symbol["LyndonBasisReplacements5Ew"<>ToString[ii]<>"MPL"]]={},{ii,5,10}];
...
loadHPL2E[weight_:5]:=Module[{currentWeight=If[ValueQ[HPL2Eweight],HPL2Eweight,5]},
  If[!ValueQ[HPL2Eweight],
     numHfuncs[3]=Length[Get["weight_3_irreducible_functions.dat"]];
     numHfuncs[4]=Length[Get["weight_4_irreducible_functions.dat"]]];
  Do[Clear[Evaluate[Symbol["invertArgument2Ew"<>ToString[n]<>"HPL"]]];
     ...
     If[n<8,numHfuncs[n]=Length[Get["weight_"<>ToString[n]<>"_irreducible_functions.dat"]]];
     Get[$MathematicaLibrary<>"/Function Library/Identities/LyndRed/Local Binaries/LyndRed2Ew"<>ToString[n]<>"_"<>ToString[Floor[$VersionNumber]]<>".mx"];
...
loadHPL3E[weight_:5]:=Module[{currentWeight=If[ValueQ[HPL3Eweight],HPL3Eweight,5]},
  ...
  Do[Clear[Evaluate[Symbol["invertArgument3Ew"<>ToString[n]<>"HPL"]]];
     ...
     Get[$MathematicaLibrary<>"/Function Library/Identities/LyndRed/Local Binaries/LyndRed3Ew"<>ToString[n]<>"_"<>ToString[Floor[$VersionNumber]]<>".mx"];
...
loadMPL5E[weight_:5]:=Module[{currentWeight=If[ValueQ[MPL5Eweight],MPL5Eweight,5]},
  ...
  Do[Get[$MathematicaLibrary<>"/Function Library/Identities/LyndRed/Local Binaries/LyndRed3Ew"<>ToString[n]<>"_"<>ToString[Floor[$VersionNumber]]<>".mx"];
     Get[$MathematicaLibrary<>"/Function Library/Identities/LyndRed/Local Binaries/LyndRed4Ew "<>ToString[n]<>"_"<>ToString[Floor[$VersionNumber]]<>".mx"];
     Get[$MathematicaLibrary<>"/Function Library/Identities/LyndRed/Local Binaries/LyndRed5Ew"<>ToString[n]<>"_"<>ToString[Floor[$VersionNumber]]<>".mx"];
     Get[$MathematicaLibrary<>"/Function Library/Weight "<>ToString[n]<>"/Local Binaries/weight_"<>ToString[n]<>"_irreducible_GI_functions.mx"];
...
loadLimits[DS,weight_:5]:=Module[{currentWeight=If[ValueQ[DSLimitsWeight],DSLimitsWeight,5]},
  If[!ValueQ[DSLimitsWeight],
     numDSfuncs[3]=Length[Get["weight_ 3_irreducible_double_scaling_functions.dat"]];
     numDSfuncs[4]=Length[Get["weight_ 4_irreducible_double_scaling_functions.dat"]];
     toLineE[DS5[a_],line_]:=Print[ToString[DS5[a]]<>" "<>ToString[line]<>" limit not loaded "];
     toLineE[DS6[a_],line_]:=Print[ToString[DS6[a]]<>" "<>ToString[line]<>" limit not loaded "];
     toLineE[DS7[a_],line_]:=Print[ToString[DS7[a]]<>" "<>ToString[line]<>" limit not loaded "];
     toLineE[DS8[a_],line_]:=Print[ToString[DS8[a]]<>" "<>ToString[line]<>" limit not loaded "];
     Get["uwLimits.m"]]
  Do[numDSfuncs[n]=Length[Get["weight_ "<>ToString[n]<>"_irreducible_double_scaling_functions.dat"]];
     Get["weight_ "<>ToString[n]<>"_ds_euclidean_lines.mx"];
     ,{n,currentWeight,weight}];
  If[!ValueQ[DSLimitsWeight]\[Or]DSLimitsWeight<weight,
     Unprotect[DSLimitsWeight];
     DSLimitsWeight=weight;
     Protect[DSLimitsWeight];
     Print["Double Scaling limits loaded through weight "<>ToString[DSLimitsWeight]];
     ,Print["Double Scaling limits already loaded through weight "<>ToString[DSLimitsWeight]]]]/;10>=weight>=5;
...
GredL[{},b_]:=1;
HredL[{},b_]:=1;
GredL[A_,b_]:=GredL[A,b]=Block[{Atemp,vars,FFF}, vars=FFF/@SortBy[Union[A],LyndOrder]; Atemp=(FFF/@A)/. Thread[vars->Array[lyn,Length[vars]]]; If[Length[vars]>1,Expand[Evaluate[Symbol["LyndRed "<>ToString[Length[vars]]<>"Ew "<>ToString[Max[Length[Atemp],5]]]][Atemp]/. Thread[Array[lyn,Length[vars]]->vars]/. FFF[C__]:>C/. QR[B__]:>G[{B},b]],G[A,b]]];
HredL[A_,b_]:=HredL[A,b]=Block[{Atemp,vars,FFF}, vars=FFF/@SortBy[Union[A],LyndOrder]; Atemp=(FFF/@A)/. Thread[vars->Array[lyn,Length[vars]]]; If[Length[vars]>1,Expand[Evaluate[Symbol["LyndRed "<>ToString[Length[vars]]<>"Ew "<>ToString[Max[Length[Atemp],5]]]][Atemp]/. Thread[Array[lyn,Length[vars]]->vars]/. FFF[C__]:>C/. QR[B__]:>HPL[{B},b]],HPL[A,b]]];
